...
Versões atuais: questionários `hm-paciente-2026.1` / `hm-psicologo-2026.1`, algoritmo `hm-match-1.0.0`.
...
| 9. Resultado ao usuário (sem “94%”) | Faixas + razões: `band`/`explain`; UI: [`PatientDashboardPage.tsx`](../../../frontend/src/pages/PatientDashboardPage.tsx) |
...
1. **Hard filters** — o profissional sai da lista (e é contado em `excluded`) se:
   - modalidade incompatível (`P20` × `S14`); presencial exige a **mesma cidade** (`P20A` × `S15`, sem acento/maiúsculas);
...
| Estilo terapêutico | 20% | Média da `similarity` entre: `6−P06`↔`S06` (estrutura), `P05`↔`S09`, `P11`↔`S08`, `P07`↔`S07`, `P10`↔`S12`, `P09`↔`S11`, `P08`↔`S10` |
| Experiência específica | 15% | 85% nível declarado em `S04` nas demandas do paciente (mesmos pesos) + 15% tempo de prática (`S01`) |
| Relação terapêutica | 15% | `P28`(+`P12`)↔`S19`, `P29`↔`S20`, `P30`(+`P03`)↔`S21`, `P15`↔`S20` (½), `P16`↔`S09` (½) |
| Disponibilidade | 10% | `min(1, períodos em comum / 3)` |
| Modalidade/local | 10% | 1 quando a preferência é atendida; 0,85 se “tanto faz” e só um formato é possível |
| Preferências pessoais | 5% | Fração das preferências declaradas (`P17`, `P18`) atendidas; 1 se não houver preferência |

3. **Score** = soma ponderada → ordena a lista. A interface mostra só a **faixa** (alta ≥ 0,75 · boa ≥ 0,55 · possível) e as **razões**, nunca uma porcentagem.
...
- **Cidade do paciente (`P20A`).** O filtro de localização da seção 2 precisa da cidade do paciente, que não estava no formulário. Aparece só quando a modalidade é presencial ou “tanto faz”.
